...
extern "C"
{
    ...
    //在稀疏矩阵填写数据
    void DLLEXPORT fillSparseMatrix(LargeSparseMatrixMemory<float>* sparseMatrix, float* data, int* indices, int* indptr, int num_rows, int num_values){
...
//        memcpy(vector->value, values, len * sizeof(float));
    ...
    FMModel* DLLEXPORT createFMModel(int num_attribute, int num_factor, bool is_use_w0, bool is_use_w, double init_stdev, double reg0, double regw, double regv){
