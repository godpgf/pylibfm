...
class FMModel{
    ...
    public:
        ...
        double predict(SparseRow<float>& x, DVector<double> &sum, DVector<double> &sum_sqr);
...
double FMModel::predict(SparseRow<float>& x, DVector<double> &sum, DVector<double> &sum_sqr) {
        double result = 0;
        if (is_use_w0) {
                result += w0;
        }
